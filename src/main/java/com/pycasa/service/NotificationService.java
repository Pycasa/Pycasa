...
public class NotificationService {
    ...
    public void unregister(Session session) {
        ...
        LOG.debugf("WebSocket client disconnected: %s (total: %d)", session.getId(), sessions.size());
    }

    /**
     * Broadcast a typed event to all connected WebSocket clients AND persist it to the DB.
     * Progress events (scan:progress, ai:progress) are broadcast but NOT persisted.
     */
    public void broadcast(String type, Map<String, Object> payload) {
        ...
        try {
            ...
            for (Session s : sessions) {
                ...
                try {
                    // Use synchronous send — works reliably from any thread including virtual threads
    ...
    public void scanProgress(int scanned, int total, String currentFile) {
        broadcast("scan:progress", Map.of(
                "scanned", scanned,
                "total", total,
