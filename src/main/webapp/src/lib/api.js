...
class ApiClient {
  ...
  folders = {
    ...
    addMonitored: (path, label) => this.request('/folders', {
      method: 'POST',
    ...
    browse: () => this.request('/folders/browse', { method: 'POST' }),
    getTrashPath: () => this.request('/folders/trash-path'),
    updateTrashPath: (path) => this.request('/folders/trash-path', {
      method: 'POST',
  ...
  images = {
    ...
    delete: (folderId, path) => this.request(`/images?folder_id=${folderId}&path=${encodeURIComponent(path)}`, {
      method: 'DELETE',
    }),
  ...
  // AI API
  ai = {
    analyse: (imagePath) => this.request('/ai/analyse', {
      method: 'POST',
    ...
    ping: (url, signal = null) => this.request('/ai/ping', {
      method: 'POST',
      body: JSON.stringify({ url }),
      signal,
    }),
    ocr: (imagePath) => this.request('/ai/ocr', {
      method: 'POST',
      body: JSON.stringify({ image_path: imagePath }),
    }),
    batchAnalyse: (rerun = false) => this.request('/ai/batch-analyse', {
      method: 'POST',
